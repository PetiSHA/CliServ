...
public class UDPEchoServer {
	...
	void start(int port) throws SocketException
	{
		...
		int size = 0;
		//A COMPLETER
		//INITIALISER L'ATTRIBUT serverSocket ET CONFIGURER
		//LA SOCKET EN MODE SERVEUR.
		//RÉCUPERER DANS size LA TAILLE DU BUFFER INTERNE.
		serverSocket = new DatagramSocket(null);
		serverSocket.bind(new InetSocketAddress(port));
		
		size = serverSocket.getReceiveBufferSize();
	...
	{
		//A COMPLETER QUESTION 2.2.1 puis 2.3.1
		incrMessageCount();
				
		String message = new String(packet.getData(), packet.getOffset(), 
				packet.getLength());
		System.out.println("Message numéro "+ receivedMessages + " (" + packet.getAddress() + ":" + packet.getPort() + ") " + message);
